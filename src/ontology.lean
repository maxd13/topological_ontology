...
-- Next we must talk about properties and relations
section relations
...
-- an exemplifiable property is positive if in any possible world in which an entity has the property,
-- there exists something in that world (an accident) to ground the property.
class property.positive : Prop := 
...
-- a positive property is proper if it "talks about" its subject
@[reducible]
def property.proper  :=
    ∃ (h : p.positive),
    ∀ (e : ω.entity) (h₂ : (p e).nonempty), 
    let r := (entity.mk (p e) (by apply h.axiom₁; exact e) h₂) in
    e.perfect → r.subsists e ∧ e.imperfect → p.essential_of e 
