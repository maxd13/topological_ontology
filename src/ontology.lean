...
-- We defer full philosophical explanation to the intensional namespace (up next).
section entities 
...
It would be easy to turn our topological ontology into an extensional theory
by identifying non-empty existential events to be possible extensional entities, and their
extensionality principle would be readily and naturally deducible from the extensionality
principle for sets. This will indeed be the primary focus of our theory, but to demonstrate 
its generality we must also discuss the introduction of primitive intensional entities via 
an intensional extension to our theory.

We would want to primarily focus on 
...
namespace intensional
  ...
  structure iontology extends ontology :=
    (ientity : Type*)
    [ene : nonempty ientity]
    (map : ientity → to_ontology.event)
    (axiom₁ : t = (generate_from $ range map))

  def iontology.up (Ω : iontology) := Ω.to_ontology
...
section ontology
 ...
 def generated_by (Ω : intensional.iontology) : Prop := Ω.up = ω'
